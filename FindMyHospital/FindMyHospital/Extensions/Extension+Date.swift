...
//  WOVO
//
//  Created by Esa Sengupta on 21/06/19.
...
enum DateFormat: String {
    case fullDateFormat = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"
    case monthDayYear = "MMM d, yyyy"
    case dayMonthYear = "dd MMM yyyy"
    case yearMonthDay = "yyyy-MM-dd"
    case simpleMonthDayYear = "MM/dd/yyyy"
    case simpleDayMonthYear = "dd/MM/yyyy"
    case dayDateMonth = "EEE d, MMM"
    case monthDayYearHourMinuteSecond = "MM/dd/yyyy HH:mm:ss"
    case monthDayYearHourMinute = "MM/dd/yyyy HH:mm"
    case timeFormat24Hr = "HH:mm:ss"
    case timeFormat12Hr = "hh:mm a"
    case guulpayDateFormat = "EEE, MMM dd, yyyy hh:mm a"
}
...
public extension Date {
    ...
    static func shortMonthSymbols() -> [String] {
        return Date.cachedFormatter().shortMonthSymbols!
    }
    ...
    func toString(dateStyle: DateFormatter.Style, timeStyle: DateFormatter.Style, isRelative: Bool = false, timeZone: Foundation.TimeZone = Foundation.TimeZone.current, locale: Locale = Locale.current) -> String {
    ...
    private static func cachedFormatter(_ dateStyle: DateFormatter.Style, timeStyle: DateFormatter.Style, doesRelativeDateFormatting: Bool, timeZone: Foundation.TimeZone = Foundation.TimeZone.current, locale: Locale = Locale.current) -> DateFormatter {
...
extension Date {
    ...
    func UTCToLocal() -> Date {
        let formatter = DateFormatter()
        formatter.dateFormat = DateFormat.fullDateFormat.rawValue
        let UTCDateString = formatter.string(from: self)
        let localDateString = UTCDateString.UTCToLocal(fromDateFormat: DateFormat.fullDateFormat.rawValue, toDateFormat: DateFormat.fullDateFormat.rawValue)
    ...
    func localToUTC() -> Date {
        ...
        let UTCDateString = localDateString.localToUTC(fromDateFormat: DateFormat.fullDateFormat.rawValue, toDateFormat: DateFormat.fullDateFormat.rawValue)
        return formatter.date(from: UTCDateString)!
    }
    
    func toApplicationDateString() -> String {
        let formatter = DateFormatter()
        formatter.dateFormat = DateFormat.fullDateFormat.rawValue
        let dateString = formatter.string(from: self)
        let date = dateString.toDate(forFormat: DateFormat.fullDateFormat.rawValue)!
        if date.compare(.isThisYear) {
            return date.toString(format: .custom(DateFormat.dayDateMonth.rawValue))
        } else {
            return date.toString(format: .custom(DateFormat.monthDayYear.rawValue))
        }
    }
    
    func toApplicationTimeString() -> String {
        let formatter = DateFormatter()
        formatter.dateFormat = DateFormat.monthDayYearHourMinuteSecond.rawValue
        let dateString = formatter.string(from: self)
        return dateString.components(separatedBy: " ").last!.to12HRFormat()
    }
}
